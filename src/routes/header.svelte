	...
	{#if routeId !== '/'}
		<div>
			{#if routeId == ''}
				<a href="https://monkeytype.com" class="monkeytype" target="_blank">
					...
					<div style="height: 22px;width:22px;">
						<svg
							...
							style="isolation:isolate; fill:var(--lightTextColor);"
				...
				>
			{:else}
				<a class="btn" tabindex="-1" href={routeId === '/multiplayer' ? '/solo' : '/multiplayer'}>
...
<style>
	...
	.btn:hover {
		opacity: 1.5;
		box-shadow: 0 0 0.7rem -0.25rem var(--lightTextColor),
	...
	:global(body.dark-mode) .btn {
		...
		border: var(--lightTextColor) 1px solid;
		opacity: 1;
	}
</style>
